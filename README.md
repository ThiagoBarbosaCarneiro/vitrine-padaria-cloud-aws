...
O objetivo era construir uma aplicação funcional onde o servidor web (**Amazon EC2**) permanecesse "limpo" (*stateless*). 

**O detalhe crucial:** As imagens não poderiam ocupar espaço no disco do servidor; elas deveriam ser enviadas, armazenadas e lidas diretamente de forma segura através do **Amazon S3**.
...
### 2. Provisionamento e Setup via Terminal
Acesso ao servidor via PowerShell e gerenciamento integral via linha de comando:
...
### 3. Desenvolvimento "Terminal-First"
...
*   **Vencendo o SELinux:** O Apache inicialmente não conseguia executar comandos externos. Aprendi a analisar logs em `/var/log/httpd/error_log` e ajustar o estado do sistema.
*   **Gestão de Permissões:** Apliquei comandos de administração como `chmod` e `chown` para garantir que o servidor web pudesse manipular os diretórios necessários.
...
## 💡 Visão de Futuro
Este projeto não foi apenas sobre "fazer um site", mas sobre entender como as peças de uma infraestrutura escalável se encaixam. Foi a oportunidade de consolidar conhecimentos e iniciar oficialmente meu portfólio em Cloud Computing.
